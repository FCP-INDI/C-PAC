...
def create_aroma(tr=None, wf_name="create_aroma"):
    ...
    inputNode = pe.Node(
        util.IdentityInterface(fields=["denoise_file", "mat_file", "fnirt_warp_file"]),
        name="inputspec",
    )

    inputNode_params = pe.Node(
        util.IdentityInterface(fields=["denoise_type", "dim"]), name="params"
    )

    outputNode = pe.Node(
        util.IdentityInterface(fields=["aggr_denoised_file", "nonaggr_denoised_file"]),
        name="outputspec",
    )
