...
def cal_snr_val(measure_file):
    ...
    """
    data = nib.load(measure_file).get_fdata()
    ...
    if measure == "FD" and len(ex_vol) > 0:
        ...
        for x in ex_vol:
            plt.annotate(
                "( %d , %0.3f)" % (x, arr[x]),
                xy=(x, arr[x]),
                arrowprops={"facecolor": "black", "shrink": 0.0},
            )
...
def gen_carpet_plt(gm_mask, wm_mask, csf_mask, functional_to_standard, output):
    ...
    func = nib.load(functional_to_standard).get_fdata()
    gm_voxels = func[nib.load(gm_mask).get_fdata().astype(bool)]
    wm_voxels = func[nib.load(wm_mask).get_fdata().astype(bool)]
    csf_voxels = func[nib.load(csf_mask).get_fdata().astype(bool)]
    del func
    ...
    gs = mgs.GridSpecFromSubplotSpec(
        1, 2, subplot_spec=mgs.GridSpec(1, 1)[0], width_ratios=[1, 100], wspace=0.0
    )
        ...
        for file_ in measure_file:
            ...
            if "sca_roi" in measure.lower():
                fname = os.path.basename(
                    os.path.splitext(os.path.splitext(file_)[0])[0]
                )
                if "ROI_" in fname:
...
def make_histogram(measure_file, measure):
    ...
    y, binEdges = np.histogram(
        data_flat[(np.isfinite(data_flat)) & (data_flat != 0)], bins=100
    )
...
def drop_percent(measure_file, percent):
    ...
    img = nib.load(measure_file)
    data = img.get_fdata()

    max_val = np.percentile(data[data != 0.0], percent)
    data[data >= max_val] = 0.0

    save_img = nib.Nifti1Image(data, header=img.header, affine=img.affine)
    ...
    return os.path.join(os.getcwd(), saved_name)
...
def montage_axial(overlay, underlay, png_name, cbar_name):
    ...
    if isinstance(overlay, list):
        ...
        for ov in overlay:
            ...
            pngs.append(
                make_montage_axial(ov, underlay, fname + "_" + png_name, cbar_name)
            )
    else:
...
def make_montage_axial(overlay, underlay, png_name, cbar_name):
    ...
    Y = nib.load(underlay).get_fdata()
    X = nib.load(overlay).get_fdata()
    ...
    if "skull_vis" in png_name:
        X[X < 20.0] = 0.0
    if (
        "skull_vis" in png_name
        or "t1_edge_on_mean_func_in_t1" in png_name
        or "MNI_edge_on_mean_func_mni" in png_name
    ):
    ...
    if (
        ...
        or ("sca_" in png_name)
        or ("alff" in png_name)
        or ("centrality" in png_name)
        or ("dr_tempreg" in png_name)
    ):
        grid = ImageGrid(
            fig,
            111,
        ...
        grid = ImageGrid(
            fig, 111, nrows_ncols=(3, 6), share_all=True, aspect=True, direction="row"
        )
...
def montage_sagittal(overlay, underlay, png_name, cbar_name):
    ...
    if isinstance(overlay, list):
        ...
        for ov in overlay:
            fname = os.path.basename(os.path.splitext(os.path.splitext(ov)[0])[0])
            pngs.append(
                make_montage_sagittal(ov, underlay, fname + "_" + png_name, cbar_name)
            )
    else:
...
def montage_gm_wm_csf_axial(overlay_csf, overlay_wm, overlay_gm, underlay, png_name):
    ...
    z1, z2 = determine_start_and_end(Y, "axial", 0.0001)
    spacing = get_spacing(6, 3, z2 - z1)
    X_csf = nib.load(overlay_csf).get_fdata()
    X_wm = nib.load(overlay_wm).get_fdata()
    X_gm = nib.load(overlay_gm).get_fdata()
