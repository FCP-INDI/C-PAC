...
def create_montage(wf_name, cbar_name, png_name, mapnode=True):
    ...
    inputnode = pe.Node(
        util.IdentityInterface(fields=["underlay", "overlay"]), name="inputspec"
    )
...
def qa_montages(
    ...
    try:
        ...
        workflow.connect(overlay, out_file, overlay_drop_percent, "measure_file")

        montage = create_montage(
            "montage_%s_%d" % (measure, num_strat), "cyan_to_yellow", measure
        )

        node, out_file_template = strat["template-brain-for-func-derivative"]
        workflow.connect(node, out_file_template, montage, "inputspec.underlay")

        workflow.connect(
            overlay_drop_percent, "modified_measure_file", montage, "inputspec.overlay"
        )
...
def create_qc_snr(wf_name="qc_snr"):
    ...
    std_dev_anat = pe.Node(fsl.ApplyWarp(interp="trilinear"), name="std_dev_anat")
    ...
    montage_snr = create_montage("montage_snr", "red_to_blue", "snr", mapnode=False)

    wf.connect(
        snr_drop_percent, "modified_measure_file", montage_snr, "inputspec.overlay"
    )
    ...
    wf.connect(
        montage_snr, "outputspec.sagittal_png", output_node, "snr_sagittal_image"
    )
...
def create_qc_motion(wf_name="qc_motion"):
    ...
    input_node = pe.Node(
        util.IdentityInterface(fields=["motion_parameters"]), name="inputspec"
    )
...
def create_qc_fd(wf_name="qc_fd"):
    ...
    input_node = pe.Node(
        util.IdentityInterface(fields=["fd", "excluded_volumes"]), name="inputspec"
    )

    output_node = pe.Node(
        util.IdentityInterface(fields=["fd_histogram_plot"]), name="outputspec"
    )

    fd_plot = pe.Node(
        Function(
            input_names=["arr", "measure", "ex_vol"],
            output_names=["hist_path"],
...
def create_qc_carpet(wf_name="qc_carpet", output_image="qc_carpet"):
    ...
    output_node = pe.Node(
        util.IdentityInterface(fields=["carpet_plot"]), name="outputspec"
    )

    gm_resample = pe.Node(
        afni.Resample(), name="gm_resample", mem_gb=0, mem_x=(0.0115, "in_file", "t")
    )
    ...
    wm_resample = pe.Node(
        afni.Resample(), name="wm_resample", mem_gb=0, mem_x=(0.0115, "in_file", "t")
    )
    ...
    csf_resample = pe.Node(
        afni.Resample(), name="csf_resample", mem_gb=0, mem_x=(0.0115, "in_file", "t")
    )
    ...
    wf.connect(
        input_node, "functional_to_standard", carpet_plot, "functional_to_standard"
    )
...
def create_qc_skullstrip(wf_name="qc_skullstrip"):
    ...
    input_node = pe.Node(
        util.IdentityInterface(fields=["anatomical_brain", "anatomical_reorient"]),
        name="inputspec",
    )
    ...
    montage_skull = create_montage("montage_skull", "red", "skull_vis", mapnode=False)
