...
def file_node(path):
    input_node = pe.Node(util.IdentityInterface(fields=["file"]), name="inputspec")
...
def test_qc():
    ...
    c = Configuration(
        {"workingDirectory": "", "crashLogDirectory": "", "outputDirectory": ""}
    )
    ...
    for (resource, _), df in output_df_dct.items():
        strat_initial.update_resource_pool({resource: file_node(df.Filepath[0])})

    qc_montage_id_a, qc_montage_id_s, qc_hist_id, qc_plot_id = create_qc_workflow(
        workflow, c, strat_list, outputs.qc
    )
