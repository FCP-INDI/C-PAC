...
def create_merged_copefile(list_of_output_files, merged_outfile):
    ...
    try:
        subprocess.check_output(merge_string)
    except Exception as e:
        err = (
...
def create_merge_mask(merged_file, mask_outfile):
    ...
    mask_string = ["fslmaths", merged_file, "-abs", "-Tmin", "-bin", mask_outfile]

    try:
        subprocess.check_output(mask_string)
    except Exception as e:
        err = (
...
def calculate_measure_mean_in_df(model_df, merge_mask):
    ...
    return pd.merge(model_df, mm_df, how="inner", on=["Raw_Filepath"])


def check_mask_file_resolution(
    data_file, roi_mask, group_mask, out_dir, output_id=None
):
    import os
    import subprocess
    ...
    raw_file_img = nib.load(data_file)
    ...
    if raw_file_dims != roi_mask_dims:
        ...
        try:
            subprocess.check_output(resample_str)
        except Exception as e:
            err = (
...
def calculate_custom_roi_mean_in_df(model_df, roi_mask):
    ...
    return pd.merge(model_df, roi_df, how="inner", on=["Raw_Filepath"])
    ...
    for con_equation in contrasts_list:
        try:
            ...
            err = (
                ...
                % (
                    str(con_equation),
                    dmatrix_obj.design_info.column_names,
                    e,
                    output_measure,
                )
            )
...
def build_feat_model(
    ...
    from CPAC.utils.create_group_analysis_info_files import (
    ...
    out_dir = os.path.join(
        model_dir, resource_id, session_id, series_or_repeated_label, preproc_strat
    )
    ...
    if "sca_roi" in resource_id:
        out_dir = os.path.join(
            ...
            re.search(
                r"sca_ROI_(\d)+",
                os.path.splitext(
                    os.path.splitext(os.path.basename(model_df["Filepath"][0]))[0]
                )[0],
            ).group(0),
        )
    ...
    if group_config_obj.participant_list is None:
        # participant_list = os.listdir(group_config_obj.pipeline_dir)
        new_sub_file = write_new_sub_file(
            model_path, group_config_obj.pipeline_dir, new_participant_list
        )
    else:
        new_sub_file = write_new_sub_file(
            model_path, group_config_obj.participant_list, new_participant_list
        )
    ...
    merge_file = create_merged_copefile(model_df["Filepath"].tolist(), merge_outfile)

    # create merged group mask
    merge_mask_outfile = "_".join([model_name, resource_id, "merged_mask.nii.gz"])
    ...
    if "Group Mask" in group_config_obj.mean_mask:
        ...
        for unique_id, series_id, raw_filepath in zip(
            model_df["participant_id"], model_df["Series"], model_df["Raw_Filepath"]
        ):
            mask_for_means_path = os.path.join(
                individual_masks_dir,
                "%s_%s_%s_mask.nii.gz" % (unique_id, series_id, resource_id),
            )
            mask_for_means = create_merge_mask(raw_filepath, mask_for_means_path)
    ...
    if "Custom_ROI_Mean" in design_formula:
        ...
        if custom_roi_mask in (None, "None", "none", ""):
            err = (
        ...
        roi_mask = check_mask_file_resolution(
            next(iter(model_df["Raw_Filepath"])),
            custom_roi_mask,
            mask_for_means,
            model_path,
            resource_id,
        ...
        output_mask = os.path.join(model_path, "masked_%s" % os.path.basename(roi_mask))
        ...
        for col in model_df.columns:
            if "Custom_ROI_Mean_" in str(col):
                if str(col) == "Custom_ROI_Mean_1":
                    ...
                    new_design_substring = new_design_substring + " + %s" % col
        design_formula = design_formula.replace("Custom_ROI_Mean", new_design_substring)
    ...
    if group_config_obj.group_sep:
        ...
        if isinstance(group_ev, list) or "," in group_ev:
            ...
            if len(group_ev) == 2:
                ...
                for x, y, z in zip(
                    model_df[group_ev[0]], model_df[group_ev[1]], model_df[group_ev[2]]
                ):
                    if x == 1:
                        grp_vector.append(1)
                    elif y == 1:
    ...
    design_formula = patsify_design_formula(
        design_formula, cat_list, group_config_obj.coding_scheme[0]
    )
    ...
    if not preset:
        ...
        if "Sessions" in model_df:
            sess_levels = list(set(model_df["Sessions"].values))
            if len(sess_levels) > 1:
                sess_map = {sess_levels[0]: "1", sess_levels[1]: "-1"}
                if len(sess_levels) == 3:
                    sess_map.update({sess_levels[2]: "0"})
                new_sess = [
                    s.replace(s, sess_map[s]) for s in list(model_df["Sessions"].values)
                ]
                model_df["Sessions"] = new_sess
        if "Series" in model_df:
            sess_levels = list(set(model_df["Series"].values))
            if len(sess_levels) > 1:
                sess_map = {sess_levels[0]: "1", sess_levels[1]: "-1"}
                if len(sess_levels) == 3:
                    sess_map.update({sess_levels[2]: "0"})
                new_sess = [
                    s.replace(s, sess_map[s]) for s in list(model_df["Series"].values)
                ]
    ...
    write_design_matrix_csv(
        dmatrix_df, model_df["participant_id"], column_names, dmat_csv_path
    )

    # time for contrasts
    if (group_config_obj.custom_contrasts is None) or (
        group_config_obj.contrasts is None
    ):
    ...
    if preset:
        ...
        if os.path.isfile(contrast_out_path):
            ...
            if (
                contrasts_df.shape[0] > 1
                or np.count_nonzero(contrasts_df.values[0][1:]) > 0
            ):
                msg = (
