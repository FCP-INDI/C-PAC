...
def json_outdir_ingress(
    filepath: Path | str, data_label: str, json: dict
    ...
    if "CpacProvenance" in json_info:
        ...
        pipe_idx = generate_prov_string(json_info["CpacProvenance"])
        node_name = ""
...
class ResourcePool:
    ...
    def set_data(
        ...
        try:
            _res, new_pipe_idx = generate_prov_string(new_prov_list)
        except IndexError:
            msg = (
        ...
        if resource not in self.rpool.keys():
            ...
            if self.get_resource_from_prov(current_prov_list) == resource:
                ...
                pipe_idx = generate_prov_string(current_prov_list)[1]
                ...
                for idx in current_prov_list:
                    if self.get_resource_from_prov(idx) == resource:
                        if isinstance(idx, list):
                            # CHANGING PIPE_IDX, BE CAREFUL DOWNSTREAM IN THIS FUNCTION
                            pipe_idx = generate_prov_string(idx)[1]
    ...
    def derivative_xfm(self, wf, label, connection, json_info, pipe_idx, pipe_x):
        if label in self.xfm:
            ...
            for entry in json_info["CpacProvenance"]:
                if isinstance(entry, list):
                    if entry[-1].split(":")[0] == xfm_label:
                        xfm_prov = entry
                        xfm_idx = generate_prov_string(xfm_prov)[1]
                        break
            ...
            for num, xfm_entry in enumerate(xfm_info):
                ...
                new_pipe_idx = generate_prov_string(new_prov)
                self.set_data(
                    label,
                    xfm,
                    "outputspec.out_file",
        ...
        for entry in json_info["CpacProvenance"]:
            if isinstance(entry, list):
                if entry[-1].split(":")[0] == mask:
                    mask_prov = entry
                    mask_idx = generate_prov_string(mask_prov)[1]
                    break
    ...
    def ingress_freesurfer(self) -> None:
        ...
        try:
            fs_path = os.path.join(
                self.cfg.pipeline_setup["freesurfer_dir"], self.part_id
            )
        except KeyError:
        ...
        if not os.path.exists(fs_path):
            if "sub" in self.part_id:
                fs_path = os.path.join(
                    self.cfg.pipeline_setup["freesurfer_dir"],
                    self.part_id.replace("sub-", ""),
                )
            else:
                fs_path = os.path.join(
                    self.cfg.pipeline_setup["freesurfer_dir"], ("sub-" + self.part_id)
                )
            ...
            if not os.path.exists(fs_path):
                ...
                fs_path = os.path.join(
                    self.cfg.pipeline_setup["freesurfer_dir"], subj_ses
                )
                if not os.path.exists(fs_path):
                    WFLOGGER.info(
    ...
    def ingress_output_dir(self) -> None:
        ...
        for subdir in [outdir_anat, outdir_func]:
            for filepath in subdir:
                ...
                json_info, pipe_idx, node_name, data_label = json_outdir_ingress(
                ...
                if (
                    ...
                    == self.cfg.pipeline_setup["outdir_ingress"]["Template"]
                ):
                    continue
    ...
    def ingress_func_metadata(
        ...
        scan_params.inputs.set(
            pipeconfig_start_indx=self.cfg.functional_preproc["truncation"]["start_tr"],
            pipeconfig_stop_indx=self.cfg.functional_preproc["truncation"]["stop_tr"],
        )
