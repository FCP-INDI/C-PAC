...
def run_group_analysis(c, resource, subject_infos):
    
    p_id, s_ids, scan_ids, s_paths = (list(tup) for tup in zip(*subject_infos))    
    model_sub_list = [line.rstrip('\r\n').split() for line in open(c.modelFile, 'r') if not (line == '\n') and not line.startswith('#')]
    ...
    for model_sub in model_sub_list:
        ...
        gp_flow = create_group_analysis_dataflow(model, c.fTest, "gp_dataflow%s"%strgy_path)
