...
def update_template_for_anat(resolution, var, num_strat, strat, c):
    import nipype.pipeline.engine as pe 
    ...
    resampled_template = pe.Node(interface = afni.Resample(), name='resampled_{0}_template_{1}'.format(var, num_strat))
    ...
    strat.update_resource_pool({'template_' + var + '_for_anat': (resampled_template, 'out_file')})         

def update_template_for_func(resolution, var1, var2, num_strat, strat, c):
    import nipype.pipeline.engine as pe 
    import nipype.interfaces.afni as afni
    ...
    resampled_template.inputs.outputtype = 'NIFTI_GZ'
    resampled_template.inputs.resample_mode = 'Cu'
    strat.update_resource_pool({'template_' + var1 + '_for_func_' + var2: (resampled_template, 'out_file')})

def update_template_symmetric(resolution, ref_template, var, num_strat, strat, c):
    import nipype.pipeline.engine as pe 
    import nipype.interfaces.afni as afni
    ...
    resampled_template = pe.Node(interface = afni.Resample(), name='resampled_{0}_template_symmetric_{1}'.format(var, num_strat))
    resampled_template.inputs.in_file = ref_template
    resampled_template.inputs.voxel_size = resolution
    resampled_template.inputs.outputtype = 'NIFTI_GZ'
    resampled_template.inputs.resample_mode = 'Cu'
    strat.update_resource_pool({'template_symmetric_' + var: (resampled_template, 'out_file')})         
