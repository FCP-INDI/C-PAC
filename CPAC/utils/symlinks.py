...
def group_files_in_strategies(output_dir, paths):
    ...
    for path in paths:
        ...
        related_strategy = tuple(
            [
                p
                for p in pieces
                if any(p.startswith("_" + pattern) for pattern in fork_ids.keys())
            ]
        )
        strategies[related_strategy].add(path)

    # Get every file that is not affected by a strategy and add it to every other strategy
    strategies_keys = set(strategies.keys()) - set()
    for paths_without_strategy in strategies[()]:
    ...
    for strategy in strategies_keys:
        ...
        for specialized_strategy in strategies_keys:
            ...
            if (
                len(specialized_strategy) > len(strategy)
                and strategy == specialized_strategy[0 : len(strategy)]
            ):
...
def create_paths_to_symlinks(output_dir, pipeline_id, subject_id, paths):
    if len(paths) == 0:
        return {}
    ...
    for strategy, strategy_paths in strategies.items():
        ...
        for path in strategy_paths:
            ...
            output_specs = [
                sp
                for sp in output_specs
                if not any(sp.startswith("_" + pattern) for pattern in fork_ids.keys())
            ]
        ...
        for i, path_symlink in enumerate(paths_symlinks):
            ...
            if any(
                rps["group"] == group
                and rps["scan"] == path_symlink["scan"]
                and rps["name"] == path_symlink["name"]
                and rps["specs"] == path_symlink["specs"]
...
def create_symlinks(
    output_dir, pipeline_id, subject_id, paths, relative=True, symlink_dir="sym_links"
):
    ...
    try:
        ...
        mapping = create_paths_to_symlinks(output_dir, pipeline_id, subject_id, paths)
        ...
        for path, symlink in mapping.items():
            ...
            if relative:
                backtrack = os.path.join(*([".."] * (len(symlink.split("/")) - 1)))
