...
@pytest.mark.parametrize(
    "config", ["blank", "default", NEUROSTARS_23786, NEUROSTARS_24035]
)
def test_sca_config(config):
    """Test SCA config parsing."""
    if "\n" in config:
        participant_config = Configuration(yaml.safe_load(config))
        assert participant_config["seed_based_correlation_analysis", "run"] is True
    else:
        participant_config = Preconfiguration(config)
        assert participant_config["seed_based_correlation_analysis", "run"] is False
