...
def test_yaml_template():
    ...
    # Create a new YAML configuration file based on the pipeline_config_default.yml file.
    config = yaml.safe_load(open('CPAC/resources/configs/pipeline_config_default.yml', 'r'))

    new_config = create_yaml_from_template(config, 'CPAC/resources/configs/pipeline_config_default.yml')
