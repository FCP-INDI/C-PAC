...
def create_con_file(
    con_vecs, con_names, col_names, model_name, current_output, out_dir
):
    import os
...
def create_fts_file(ftest_list, con_names, model_name, current_output, out_dir):
    import os
...
def create_flame_model_files(
    ...
    if contrasts_vectors:
        ...
        if len(ftest_list) > 0:
            fts_file = create_fts_file(
                ftest_list, contrast_names, model_name, output_measure, output_dir
            )
        else:
            fts_file = None

    elif custom_contrasts_csv:
        con_file, fts_file = create_con_ftst_file(
            custom_contrasts_csv,
            model_name,
            output_measure,
            output_dir,
    ...
    mat_file = write_mat_file(
        design_matrix, output_dir, model_name, col_names, output_measure
    )

    grp_file = create_grp_file(design_matrix, grouping_vector, output_dir, model_name)
