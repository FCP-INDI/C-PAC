...
def process_pheno_file(
    pheno_file_dataframe, group_subs_dataframe, participant_id_label
):
    ...
    for i in range(0, len(subjects)):
        ...
        if sessions and series:
            ...
            try:
                row = pheno[
                    (pheno[participant_id_label] == subject)
                    & (pheno.session == session)
                    & (pheno.series == scan)
                ]
            except:
                row = pheno[
                    (pheno[participant_id_label] == int(subject))
                    & (pheno.session == session)
                    & (pheno.series == scan)
                ]
            ...
            try:
                row = pheno[
                    (pheno[participant_id_label] == subject) & (pheno.series == scan)
                ]
            except:
                row = pheno[
                    (pheno[participant_id_label] == int(subject))
    ...
    return new_pheno_df.to_dict("records")
        ...
        for val in line.values():
            ...
            for key in line.keys():
                ...
                if "categorical" in ev_selections.keys():
                    ...
                    elif key in (subject_id_label, "session", "series"):
...
def get_measure_dict(param_file):
    ...
    for m in measures:
        ...
        if m in motion_params.columns:
            ...
            for part_id, scan_id, measure_val in zip(part_ids, scan_ids, measure_vals):
...
def model_group_var_separately(
    ...
    for key in pheno_data_dict.keys():
        ...
        if (key in formula) and (key != grouping_var):
            ...
            for level in groupvar_levels:
                ...
                for val, groupvar_val in zip(
                    pheno_data_dict[key], pheno_data_dict[grouping_var]
                ):
...
def write_mat_file(
    design_matrix, output_dir, model_name, depatsified_EV_names, current_output=None
):
    import os
...
def create_grp_file(
    design_matrix, grouping_var_id_dict, output_dir, model_name, current_output=None
):
    import os
    ...
    if grouping_var_id_dict is not None:
        i = 1
...
def create_design_matrix(
    ...
    pheno_file_rows = process_pheno_file(
        pheno_file_df, participant_list_df, subject_id_label
    )
            ...
            if measure in formula:
                ...
                for pheno_row_dict in pheno_file_rows:
                    ...
                    if ("session" in pheno_row_dict.keys()) and (
                        ...
                        participant_tuple = (participant_id, session_id, series_id)
    ...
    pheno_data_dict = create_pheno_dict(
        pheno_file_rows, ev_selections, subject_id_label
    )
    ...
    if grouping_var is not None:
        pheno_data_dict, formula, grouping_var_id_dict = model_group_var_separately(
            grouping_var, formula, pheno_data_dict, ev_selections, coding_scheme
        )
    ...
    if len(column_names) >= num_subjects:
        err = (
            ...
            % (current_output, num_subjects, len(column_names), current_output)
        )
        raise Exception(err)
    ...
    write_mat_file(
        design_matrix, output_dir, model_name, depatsified_EV_names, current_output
    )

    # write the .grp file also
    create_grp_file(
        design_matrix, grouping_var_id_dict, output_dir, model_name, current_output
    )
...
def negative(dmat, a, coding, group_sep, grouping_var):
    return 0 - positive(dmat, a, coding, group_sep, grouping_var)
...
def create_fts_file(ftest_list, con_dict, model_name, current_output, out_dir):
    ...
    try:
        ...
        filepath = os.path.join(
            out_dir, "model_files", current_output, model_name + ".fts"
        )
...
def process_contrast(
    parsed_contrast, operator, ev_selections, group_sep, grouping_var, coding_scheme
):
    ...
    for EV in EVs_in_contrast:
        ...
        if "categorical" in ev_selections.keys():
            for cat_EV in ev_selections["categorical"]:
                ...
                if (cat_EV in EV) and not (gpvar in EV and "__" in EV):
                    # handle interactions
                    if ":" in EV:
                    ...
                    if coding_scheme == "Treatment":
                        cat_EV_contrast = EV.replace(
                            EV, "C(" + cat_EV + ")[T." + current_EV + "]"
                        )

                    elif coding_scheme == "Sum":
                        cat_EV_contrast = EV.replace(
                            EV, "C(" + cat_EV + ", Sum)[S." + current_EV + "]"
                        )
        ...
        if ":" in EV and len(parsed_EVs_in_contrast) == 2:
            parsed_EVs_in_contrast = [
                parsed_EVs_in_contrast[0] + ":" + parsed_EVs_in_contrast[1]
            ]

        if ":" in EV and len(parsed_EVs_in_contrast) == 3:
            parsed_EVs_in_contrast = [
                parsed_EVs_in_contrast[0],
                parsed_EVs_in_contrast[1] + ":" + parsed_EVs_in_contrast[2],
            ]
...
def run(
    ...
    if CPAC_run:
        ...
        try:
            c = Configuration(yaml.safe_load(open(os.path.realpath(group_config), "r")))
        except:
            raise Exception("Error in reading %s configuration file" % group_config)
    ...
    design_matrix, regressor_names = create_design_matrix(
        ph,
        sublist,
        ev_selections,
        formula,
    ...
    for contrast in contrasts_list:
        ...
        if ">" in parsed_contrast:
            ...
            contrasts_dict[parsed_contrast] = greater_than(
                design_matrix,
                parsed_EVs_in_contrast[0],
            ...
            if "+" in contrast_items and len(contrast_items) == 2:
                ...
                contrasts_dict[parsed_contrast] = positive(
                    design_matrix,
                    parsed_EVs_in_contrast[0],
                    coding_scheme,
                    group_sep,
                ...
                contrasts_dict[parsed_contrast] = negative(
                    design_matrix,
                    parsed_EVs_in_contrast[0],
                    coding_scheme,
                    group_sep,
                    ...
                    if "categorical" in ev_selections.keys():
                        for cat_EV in ev_selections["categorical"]:
                            if cat_EV in item:
                                if coding_scheme == "Treatment":
                                    item = item.replace(
                                        item, "C(" + cat_EV + ")[T." + item + "]"
                                    )

                                elif coding_scheme == "Sum":
                                    item = item.replace(
                                        item, "C(" + cat_EV + ", Sum)[S." + item + "]"
                                    )
    ...
    if (
        ...
        if f_tests:
            create_fts_file(
                f_tests, contrasts_dict, model_name, current_output, model_out_dir
            )
