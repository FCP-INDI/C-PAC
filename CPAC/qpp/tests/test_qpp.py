...
def test_qpp():
    ...
    for i in range(trs):
        convolved[i] = (
            np.convolve(x[i], best_template_segment[i], mode="full") / window_length
        )[:trs]

    plt.fill_between(
        range(trs),
        convolved.mean(axis=0) - convolved.std(axis=0),
        convolved.mean(axis=0) + convolved.std(axis=0),
        facecolor="red",
        alpha=0.2,
    )
    ...
    plt.plot(
        range(-window_length, 0),
        best_template_segment.mean(axis=0),
        "--",
        label="QPP",
        color="green",
    )
