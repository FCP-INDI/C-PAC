...
def detect_qpp(
    ...
    joint_mask_img = nib.load(joint_mask)
    joint_mask = joint_mask_img.get_fdata().astype(bool)
    joint_datasets_img = nib.load(joint_datasets)
    joint_datasets = joint_datasets_img.get_fdata()[joint_mask]

    def correlation_threshold(i):
        return (
            higher_correlation_threshold
            if i > correlation_threshold_iteration
            else lower_correlation_threshold
        )
    ...
    qpp_img = nib.Nifti1Image(qpp, joint_mask_img.affine)
...
def create_qpp(name="qpp", working_dir=None, crash_dir=None):
    ...
    outputspec = pe.Node(util.IdentityInterface(fields=["qpp"]), name="outputspec")
