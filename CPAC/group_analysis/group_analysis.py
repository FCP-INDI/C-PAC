...
def create_fsl_flame_wf(ftest=False, wf_name="groupAnalysis"):
    ...
    merge_mean_mask = pe.Node(interface=fsl.ImageMaths(), name="merge_mean_mask")
    ...
    grp_analysis.connect(label_zstat, "new_zstat_list", rename_zstats, "format_string")
    ...
    if ftest:
        ...
        grp_analysis.connect(fsl_flameo, "zfstats", easy_thresh_zf, "inputspec.z_stats")
        grp_analysis.connect(
            inputnode, "merge_mask", easy_thresh_zf, "inputspec.merge_mask"
        )
        grp_analysis.connect(
            inputnode, "z_threshold", easy_thresh_zf, "inputspec.z_threshold"
        )
        grp_analysis.connect(
            inputnode, "p_threshold", easy_thresh_zf, "inputspec.p_threshold"
        )
        grp_analysis.connect(
            inputnode, "parameters", easy_thresh_zf, "inputspec.parameters"
        )
        grp_analysis.connect(
            easy_thresh_zf,
            "outputspec.cluster_threshold",
            outputnode,
            "cluster_threshold_zf",
        )
        grp_analysis.connect(
            easy_thresh_zf, "outputspec.cluster_index", outputnode, "cluster_index_zf"
        )
        grp_analysis.connect(
            easy_thresh_zf,
            "outputspec.cluster_localmax_txt",
            outputnode,
            "cluster_localmax_txt_zf",
        )
        grp_analysis.connect(
            easy_thresh_zf,
            "outputspec.overlay_threshold",
            outputnode,
            "overlay_threshold_zf",
        )
        grp_analysis.connect(
            easy_thresh_zf, "outputspec.rendered_image", outputnode, "rendered_image_zf"
        )

    # calling easythresh for zstats files
    easy_thresh_z = easy_thresh("easy_thresh_z")
    grp_analysis.connect(rename_zstats, "out_file", easy_thresh_z, "inputspec.z_stats")
    grp_analysis.connect(inputnode, "merge_mask", easy_thresh_z, "inputspec.merge_mask")
    grp_analysis.connect(
        inputnode, "z_threshold", easy_thresh_z, "inputspec.z_threshold"
    )
    grp_analysis.connect(
        inputnode, "p_threshold", easy_thresh_z, "inputspec.p_threshold"
    )
    grp_analysis.connect(inputnode, "parameters", easy_thresh_z, "inputspec.parameters")
    ...
    grp_analysis.connect(get_opstring, "out_file", merge_mean_mask, "op_string")
    ...
    grp_analysis.connect(
        easy_thresh_z, "outputspec.cluster_threshold", outputnode, "cluster_threshold"
    )
    grp_analysis.connect(
        easy_thresh_z, "outputspec.cluster_index", outputnode, "cluster_index"
    )
    grp_analysis.connect(
        easy_thresh_z,
        "outputspec.cluster_localmax_txt",
    ...
    grp_analysis.connect(
        easy_thresh_z, "outputspec.overlay_threshold", outputnode, "overlay_threshold"
    )
    grp_analysis.connect(
        easy_thresh_z, "outputspec.rendered_image", outputnode, "rendered_image"
    )

    return grp_analysis


def run_feat_pipeline(
    group_config,
    merge_file,
    merge_mask,
    f_test,
    ...
    # workflow time
    wf_name = "fsl-feat_".format()
    wf = pe.Workflow(name=wf_name)

    wf.base_dir = work_dir

    wf.config["execution"] = {"hash_method": "timestamp", "crashdump_dir": log_dir}
