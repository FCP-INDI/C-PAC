...
def create_centrality_wf(
    ...
    input_node = pe.Node(
        util.IdentityInterface(fields=["in_file", "template", "threshold"]),
        name="inputspec",
    )
    input_node.inputs.threshold = threshold
    output_node = pe.Node(
        util.IdentityInterface(fields=["outfile_list"]), name="outputspec"
    )

    # Degree centrality
    if method_option == "degree_centrality":
        afni_centrality_node = pe.Node(
            DegreeCentrality(environ={"OMP_NUM_THREADS": str(num_threads)}),
    ...
    # lFCD
        ...
        afni_centrality_node = pe.Node(
            LFCD(environ={"OMP_NUM_THREADS": str(num_threads)}),
            name="afni_centrality",
            mem_gb=memory_gb,
        )
    ...
    if not ecm_gte_21_1_01:
        ...
        centrality_wf.connect(
            [
                (afni_centrality_node, sep_subbriks_node, [("out_file", "nifti_file")]),
                (sep_subbriks_node, output_node, [("output_niftis", "outfile_list")]),
            ]
        )
        ...
        centrality_wf.connect(
            [
                (
                    input_node,
                    convert_thr_node,
                    [("in_file", "datafile"), ("threshold", "p_value")],
                ),
                (
                    convert_thr_node,
                    afni_centrality_node,
