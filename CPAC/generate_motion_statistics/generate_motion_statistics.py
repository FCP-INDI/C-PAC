...
def motion_power_statistics(
    ...
    calculate_FDJ = pe.Node(
        Function(
            input_names=["in_file", "calc_from", "center"],
            output_names=["out_file", "fd"],
    ...
    calc_motion_parameters = pe.Node(
        Function(
            input_names=[
                "movement_parameters",
                "max_displacement",
            ...
            output_names=["out_file", "info", "maxdisp", "relsdisp"],
    ...
    get_all_motion_parameters = pe.Node(
        Function(
            input_names=[
                "fdj",
                "fdp",
    ...
    wf.connect(calc_motion_parameters, "maxdisp", get_all_motion_parameters, "maxdisp")
    wf.connect(
        calc_motion_parameters, "relsdisp", get_all_motion_parameters, "relsdisp"
    )
    wf.connect(calc_motion_parameters, "info", get_all_motion_parameters, "motion")
    wf.connect(cal_DVARS_strip, "DVARS_val", get_all_motion_parameters, "dvars")
    wf.connect(
        input_node, "movement_parameters", calc_motion_parameters, "movement_parameters"
    )
    wf.connect(
        input_node, "max_displacement", calc_motion_parameters, "max_displacement"
    )
    wf.connect(
        input_node, "rels_displacement", calc_motion_parameters, "rels_displacement"
    )
    wf.connect(calc_motion_parameters, "out_file", output_node, "motion_params")
    wf.connect(get_all_motion_parameters, "all_motion_val", output_node, "motion")
    wf.connect(
        get_all_motion_parameters,
        "summary_motion_power",
        output_node,
        "desc-summary_motion",
    )

    calc_power_parameters = pe.Node(
        Function(
            input_names=["fdp", "fdj", "dvars", "motion_correct_tool"],
    ...
    wf.connect(calc_power_parameters, "info", get_all_motion_parameters, "power")
...
def find_volume_center(img_file: str) -> np.ndarray:
    ...
    return 0.5 * (dim - 1) * pixdim


def gen_motion_parameters(
    ...
    def max_relative(v):
        return np.max(abs_relative(v))

    def avg_relative(v):
        return np.mean(abs_relative(v))
...
def gen_power_parameters(
    ...
    if fdp:
        ...
        if motion_correct_tool == "3dvolreg":
            ...
            info = [("MeanFD_Power", meanFD_Power), ("MeanDVARS", meanDVARS)]
...
class ImageTo1DInputSpec(AFNICommandInputSpec):
    ...
    out_file = File(
        name_template="%s_3DtoT1.1D",
        desc="output 1D file name",
        argstr="-prefix %s",
        name_source="in_file",
        keep_extension=True,
    )
    ...
    method = traits.Enum(*_methods, argstr="-method %s")
...
def calculate_DVARS(func_brain, mask):
    ...
    rest_data = nib.load(func_brain).get_fdata().astype(np.float32)
...
def get_allmotion(fdj, fdp, maxdisp, motion, power, relsdisp=None, dvars=None):
    ...
    summary_motion_pow_df.to_csv(
        summary_motion_power, sep="\t", header=True, index=False
    )
