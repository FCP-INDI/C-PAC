...
def process_segment_map(wf_name, use_priors, use_custom_threshold, reg_tool):
    ...
    outputNode = pe.Node(
        util.IdentityInterface(fields=["segment_mask", "probability_tissue_map"]),
        name="outputspec",
    )
    ...
    if use_priors:
        apply_xfm = apply_transform(
            "seg_tissue_priors_template_to_T1", reg_tool=reg_tool
        )
        apply_xfm.inputs.inputspec.interpolation = "NearestNeighbor"

        preproc.connect(inputNode, "tissue_prior", apply_xfm, "inputspec.input_image")
        preproc.connect(inputNode, "brain", apply_xfm, "inputspec.reference")
        preproc.connect(
            inputNode, "template_to_T1_xfm", apply_xfm, "inputspec.transform"
        )
    ...
    if use_custom_threshold:
        segmentmap_threshold = pe.Node(
            interface=fsl.ImageMaths(), name="threshold_segmentmap_%s" % (wf_name)
        )
        preproc.connect(
            inputNode,
        ...
        binarize_threshold_segmentmap = pe.Node(
            interface=fsl.ImageMaths(), name="binarize_%s" % (wf_name)
        )
...
def tissue_mask_template_to_t1(wf_name, use_ants):
    ...
    outputNode = pe.Node(
        util.IdentityInterface(fields=["segment_mask_temp2t1"]), name="outputspec"
    )
    ...
    if use_ants:
        ...
        preproc.connect(
            inputNode, "standard2highres_init", collect_linear_transforms, "in1"
        )
        preproc.connect(
            inputNode, "standard2highres_rig", collect_linear_transforms, "in2"
        )
        preproc.connect(
            inputNode, "standard2highres_mat", collect_linear_transforms, "in3"
        )
        ...
        preproc.connect(
            collect_linear_transforms, "out", check_transform, "transform_list"
        )
        ...
        preproc.connect(
            inputNode, "tissue_mask_template", tissueprior_mni_to_t1, "input_image"
        )

        preproc.connect(
            tissueprior_mni_to_t1, "output_image", outputNode, "segment_mask_temp2t1"
        )
        ...
        # mni to t1
        preproc.connect(
            inputNode, "tissue_mask_template", tissueprior_mni_to_t1, "in_file"
        )
        preproc.connect(inputNode, "brain", tissueprior_mni_to_t1, "reference")
        preproc.connect(
            inputNode, "standard2highres_mat", tissueprior_mni_to_t1, "in_matrix_file"
        )

        preproc.connect(
            tissueprior_mni_to_t1, "out_file", outputNode, "segment_mask_temp2t1"
        )
...
def create_seg_preproc_antsJointLabel_method(wf_name="seg_preproc_templated_based"):
    ...
    outputNode = pe.Node(
        util.IdentityInterface(fields=["csf_mask", "gm_mask", "wm_mask"]),
        name="outputspec",
    )
    ...
    preproc.connect(
        inputNode, "anatomical_brain", seg_preproc_antsJointLabel, "anatomical_brain"
    )
    preproc.connect(
        inputNode,
        "anatomical_brain_mask",
    ...
    preproc.connect(inputNode, "csf_label", pick_tissue, "csf_label")
    preproc.connect(inputNode, "gm_label", pick_tissue, "gm_label")
    preproc.connect(inputNode, "wm_label", pick_tissue, "wm_label")

    preproc.connect(pick_tissue, "csf_mask", outputNode, "csf_mask")
    preproc.connect(pick_tissue, "gm_mask", outputNode, "gm_mask")
    preproc.connect(pick_tissue, "wm_mask", outputNode, "wm_mask")
...
def tissue_seg_fsl_fast(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    segment = pe.Node(
        ...
        mem_gb=3.48,
        mem_x=(3444233104315183 / 19342813113834066795298816, "in_files"),
    )
    ...
    use_custom_threshold = (
        cfg["segmentation"]["tissue_segmentation"]["FSL-FAST"]["thresholding"]["use"]
        == "Custom"
    )

    use_priors = cfg["segmentation"]["tissue_segmentation"]["FSL-FAST"]["use_priors"][
        "run"
    ]
    ...
    process_csf = process_segment_map(
        f"CSF_{pipe_num}", use_priors, use_custom_threshold, reg_tool
    )
    process_csf.inputs.inputspec.threshold = cfg["segmentation"]["tissue_segmentation"][
        "FSL-FAST"
    ]["thresholding"]["Custom"]["CSF_threshold_value"]

    get_pve_csf = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"get_pve_csf_{pipe_num}"
    )
    get_pve_csf.inputs.args = "-thr 0.5 -uthr 1.5 -bin"
    wf.connect(segment, "partial_volume_map", get_pve_csf, "in_file")

    get_pve_gm = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"get_pve_gm_{pipe_num}"
    )
    get_pve_gm.inputs.args = "-thr 1.5 -uthr 2.5 -bin"
    wf.connect(segment, "partial_volume_map", get_pve_gm, "in_file")

    get_pve_wm = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"get_pve_wm_{pipe_num}"
    )
    ...
    process_gm = process_segment_map(
        f"GM_{pipe_num}", use_priors, use_custom_threshold, reg_tool
    )
    process_gm.inputs.inputspec.threshold = cfg["segmentation"]["tissue_segmentation"][
        "FSL-FAST"
    ]["thresholding"]["Custom"]["GM_threshold_value"]

    if use_priors:
        node, out = strat_pool.get_data("GM-path")
        wf.connect(node, out, process_gm, "inputspec.tissue_prior")

    process_wm = process_segment_map(
        f"WM_{pipe_num}", use_priors, use_custom_threshold, reg_tool
    )
    process_wm.inputs.inputspec.threshold = cfg["segmentation"]["tissue_segmentation"][
        "FSL-FAST"
    ]["thresholding"]["Custom"]["WM_threshold_value"]
    ...
    wf.connect(
        segment,
        ("tissue_class_files", pick_wm_class_0),
        process_csf,
        "inputspec.tissue_class_file",
    )
    wf.connect(
        segment,
        ("probability_maps", pick_wm_prob_0),
        process_csf,
    ...
    wf.connect(
        segment,
        ("tissue_class_files", pick_wm_class_1),
        process_gm,
        "inputspec.tissue_class_file",
    )
    wf.connect(
        segment,
        ("probability_maps", pick_wm_prob_1),
        process_gm,
        "inputspec.probability_tissue_map",
    )
    ...
    wf.connect(
        segment,
        ("tissue_class_files", pick_wm_class_2),
        process_wm,
        "inputspec.tissue_class_file",
    )
    wf.connect(
        segment,
        ("probability_maps", pick_wm_prob_2),
        process_wm,
        "inputspec.probability_tissue_map",
    )
    ...
    outputs = {
        ...
        f"{long}label-GM_probseg": (segment, ("probability_maps", pick_wm_prob_1)),
        f"{long}label-WM_probseg": (segment, ("probability_maps", pick_wm_prob_2)),
        f"{long}label-CSF_mask": (segment, ("tissue_class_files", pick_wm_class_0)),
        f"{long}label-GM_mask": (segment, ("tissue_class_files", pick_wm_class_1)),
        f"{long}label-WM_mask": (segment, ("tissue_class_files", pick_wm_class_2)),
        f"{long}label-CSF_desc-preproc_mask": (process_csf, "outputspec.segment_mask"),
        f"{long}label-GM_desc-preproc_mask": (process_gm, "outputspec.segment_mask"),
        f"{long}label-WM_desc-preproc_mask": (process_wm, "outputspec.segment_mask"),
...
def tissue_seg_T1_template_based(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    csf_template2t1.inputs.inputspec.tissue_mask_template = cfg["segmentation"][
        "tissue_segmentation"
    ]["Template_Based"]["CSF"]

    gm_template2t1 = tissue_mask_template_to_t1(f"GM_{pipe_num}", use_ants)
    gm_template2t1.inputs.inputspec.tissue_mask_template = cfg["segmentation"][
        "tissue_segmentation"
    ]["Template_Based"]["GRAY"]

    wm_template2t1 = tissue_mask_template_to_t1(f"WM_{pipe_num}", use_ants)
    wm_template2t1.inputs.inputspec.tissue_mask_template = cfg["segmentation"][
        "tissue_segmentation"
    ...
    wf.connect(node, out, csf_template2t1, "inputspec.standard2highres_mat")
    wf.connect(node, out, wm_template2t1, "inputspec.standard2highres_mat")
    wf.connect(node, out, gm_template2t1, "inputspec.standard2highres_mat")

    outputs = {
        "label-CSF_mask": (csf_template2t1, "outputspec.segment_mask_temp2t1"),
...
def tissue_seg_EPI_template_based(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {
        ...
        "space-bold_label-GM_mask": (gm_template2t1, "outputspec.segment_mask_temp2t1"),
        "space-bold_label-WM_mask": (wm_template2t1, "outputspec.segment_mask_temp2t1"),
    }
...
def tissue_seg_ants_prior(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    seg_preproc_ants_prior_based.inputs.inputspec.template_brain_list = cfg[
        "segmentation"
    ]["tissue_segmentation"]["ANTs_Prior_Based"]["template_brain_list"]
    seg_preproc_ants_prior_based.inputs.inputspec.template_segmentation_list = cfg[
        "segmentation"
    ]["tissue_segmentation"]["ANTs_Prior_Based"]["template_segmentation_list"]

    seg_preproc_ants_prior_based.inputs.inputspec.csf_label = cfg["segmentation"][
        "tissue_segmentation"
    ]["ANTs_Prior_Based"]["CSF_label"]

    seg_preproc_ants_prior_based.inputs.inputspec.gm_label = cfg["segmentation"][
        "tissue_segmentation"
    ]["ANTs_Prior_Based"]["GM_label"]

    seg_preproc_ants_prior_based.inputs.inputspec.wm_label = cfg["segmentation"][
        "tissue_segmentation"
    ]["ANTs_Prior_Based"]["WM_label"]

    node, out = strat_pool.get_data("desc-brain_T1w")
    wf.connect(node, out, seg_preproc_ants_prior_based, "inputspec.anatomical_brain")

    node, out = strat_pool.get_data(
        ["space-T1w_desc-brain_mask", "space-T1w_desc-acpcbrain_mask"]
    )
    wf.connect(
        node, out, seg_preproc_ants_prior_based, "inputspec.anatomical_brain_mask"
    )

    outputs = {
        "label-CSF_mask": (seg_preproc_ants_prior_based, "outputspec.csf_mask"),
...
def tissue_seg_freesurfer(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    fs_aseg_to_native = pe.Node(
        interface=freesurfer.ApplyVolTransform(), name=f"fs_aseg_to_native_{pipe_num}"
    )
    ...
    wf.connect(fs_aseg_to_native, "transformed_file", fs_aseg_to_nifti, "in_file")

    pick_tissue = pe.Node(
        pick_tissue_from_labels_file_interface(), name=f"select_fs_tissue_{pipe_num}"
    )

    pick_tissue.inputs.csf_label = cfg["segmentation"]["tissue_segmentation"][
        "FreeSurfer"
    ]["CSF_label"]
    pick_tissue.inputs.gm_label = cfg["segmentation"]["tissue_segmentation"][
        "FreeSurfer"
    ]["GM_label"]
    pick_tissue.inputs.wm_label = cfg["segmentation"]["tissue_segmentation"][
        "FreeSurfer"
    ]["WM_label"]
    ...
    if cfg["segmentation"]["tissue_segmentation"]["FreeSurfer"]["erode"] > 0:
        for tissue in ["csf", "wm", "gm"]:
            erode_tissues[tissue] = pe.Node(
                interface=freesurfer.model.Binarize(), name=f"erode_{tissue}_{pipe_num}"
            )
            erode_tissues[tissue].inputs.match = [1]
            erode_tissues[tissue].inputs.erode = cfg["segmentation"][
                "tissue_segmentation"
            ]["FreeSurfer"]["erode"]
            wf.connect(pick_tissue, f"{tissue}_mask", erode_tissues[tissue], "in_file")
