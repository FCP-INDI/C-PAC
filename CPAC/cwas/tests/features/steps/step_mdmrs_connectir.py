...
def step(context, mdir, factor):
    ...
    context.r_F_perms = np.array(
        robjects.r("as.matrix(attach.big.matrix('%s'))" % ffile)
    )
    # Fstats
...
def step(context):
    ...
    context.Fs, context.ps = calc_mdmrs(
        context.dmats[context.voxs], context.regressors, context.cols, context.perms
    )
...
def step(context):
    context.voxs = [0]
    ps, Fs, F_perms, _ = mdmr(
        context.dmats[0].reshape(context.nobs**2, 1),
        context.regressors,
        context.cols,
        context.perms,
    )
