...
def create_cwas(name="cwas", working_dir=None, crash_dir=None):
    ...
    outputspec = pe.Node(
        util.IdentityInterface(
            fields=["F_map", "p_map", "neglog_p_map", "one_p_map", "z_map"]
        ),
        name="outputspec",
    )
    ...
    jmask = pe.Node(
        Function(
            input_names=["subjects", "mask_file"],
    ...
    mcwasb = pe.Node(
        Function(
            input_names=["cwas_batches", "mask_file", "z_score", "permutations"],
            output_names=["F_file", "p_file", "neglog_p_file", "one_p_file", "z_file"],
