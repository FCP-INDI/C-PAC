...
def easy_thresh(wf_name):
    ...
    inputnode = pe.Node(
        util.IdentityInterface(
            fields=["z_stats", "merge_mask", "z_threshold", "p_threshold", "parameters"]
        ),
        name="inputspec",
    )
    ...
    smooth_estimate = pe.MapNode(
        interface=fsl.SmoothEstimate(), name="smooth_estimate", iterfield=["zstat_file"]
    )

    # run clustering after fixing stats header for talspace
    zstat_mask = pe.MapNode(
        interface=fsl.MultiImageMaths(), name="zstat_mask", iterfield=["in_file"]
    )
    ...
    overlay = pe.MapNode(
        interface=fsl.Overlay(), name="overlay", iterfield=["stat_image", "stat_thresh"]
    )
    ...
    easy_thresh.connect(
        inputnode, "parameters", get_backgroundimage2, "file_parameters"
    )
    ...
    easy_thresh.connect(
        cluster, "localmax_txt_file", outputnode, "cluster_localmax_txt"
    )
