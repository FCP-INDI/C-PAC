...
def test_registration():
    from CPAC.pipeline import nipype_pipeline_engine as pe
    from ..registration import create_nonlinear_register, create_register_func_to_mni
    ...
    mni_workflow.connect(
        nr, "outputspec.nonlinear_xfm", func2mni, "inputspec.anat_to_mni_xfm"
    )
...
def test_registration_lesion():
    ...
    anat_file = (
        "/bids_dataset/sub-0027228/ses-1/anat/sub-0027228_ses-1_run-1_T1w.nii.gz"
    )
    lesion_file = "/bids_dataset/sub-0027228/ses-1/anat/sub-0027228_ses-1_run-1_T1w_lesion-mask.nii.gz"
    mni_brain_file = (
        f'{os.environ.get("FSLDIR")}/data/standard/' 'MNI152_T1_3mm_brain.nii.gz'
    )
    ...
    ants_reg_anat_mni = create_wf_calculate_ants_warp(
        "anat_mni_ants_register", 0, num_threads=4
    )
    ...
    wf.connect(
        anat_preproc, "outputspec.reorient", ants_reg_anat_mni, "inputspec.moving_brain"
    )
