...
from CPAC.utils.utils import extract_output_mean, get_fisher_zscore, get_zscore


def z_score_standardize(wf_name, input_image_type="func_derivative", opt=None):
...
def fisher_z_score_standardize(
    wf_name, label, input_image_type="func_derivative", opt=None
):
    wf = pe.Workflow(name=wf_name)
    ...
    inputnode = pe.Node(
        util.IdentityInterface(fields=["correlation_file", "timeseries_oned"]),
        name="inputspec",
    )

    fisher_z_score_std = get_fisher_zscore(label, map_node, "fisher_z_score_std")
    wf.connect(
        inputnode, "correlation_file", fisher_z_score_std, "inputspec.correlation_file"
    )

    wf.connect(
        inputnode, "timeseries_oned", fisher_z_score_std, "inputspec.timeseries_one_d"
    )

    outputnode = pe.Node(util.IdentityInterface(fields=["out_file"]), name="outputspec")

    wf.connect(
        fisher_z_score_std, "outputspec.fisher_z_score_img", outputnode, "out_file"
    )
...
def calc_avg(workflow, output_name, strat, num_strat, map_node=False):
    ...
    if map_node:
        ...
        mean_to_csv = pe.MapNode(
            function.Function(
                input_names=["in_file", "output_name"],
