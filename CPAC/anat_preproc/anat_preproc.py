...
def acpc_alignment(
    config=None, acpc_target="whole-head", mask=False, wf_name="acpc_align"
):
    ...
    if config.anatomical_preproc["acpc_alignment"]["FOV_crop"] == "robustfov":
        robust_fov = pe.Node(
            interface=fsl_utils.RobustFOV(), name="anat_acpc_1_robustfov"
        )
    ...
    convert_fov_xfm = pe.Node(
        interface=fsl_utils.ConvertXFM(), name="anat_acpc_2_fov_convertxfm"
    )
    ...
    concat_xfm = pe.Node(interface=fsl_utils.ConvertXFM(), name="anat_acpc_4_concatxfm")
    ...
    apply_xfm = pe.Node(interface=fsl.ApplyWarp(), name="anat_acpc_6_applywarp")
    ...
    if acpc_target == "brain":
        apply_xfm_brain = pe.Node(
            interface=fsl.ApplyWarp(), name="anat_acpc_brain_6_applywarp"
        )
        ...
        preproc.connect(
            inputnode, "template_brain_for_acpc", apply_xfm_brain, "ref_file"
        )
    ...
    if mask:
        apply_xfm_mask = pe.Node(
            interface=fsl.ApplyWarp(), name="anat_mask_acpc_7_applywarp"
        )
        ...
        preproc.connect(
            inputnode, "template_brain_for_acpc", apply_xfm_mask, "ref_file"
        )
...
def T2wToT1wReg(wf_name="T2w_to_T1w_reg"):
    ...
    outputnode = pe.Node(
        util.IdentityInterface(fields=["T2w_to_T1w"]), name="outputspec"
    )
    ...
    T2w2T1w = pe.Node(interface=fsl.ApplyWarp(), name="T2w2T1w_applywarp")
...
def BiasFieldCorrection_sqrtT1wXT1w(config=None, wf_name="biasfield_correction_t1t2"):
    ...
    inputnode = pe.Node(
        util.IdentityInterface(fields=["T1w", "T1w_brain", "T2w"]), name="inputspec"
    )
    ...
    T1wmulT2w = pe.Node(interface=fsl.MultiImageMaths(), name="T1wmulT2w")
    ...
    T1wmulT2w_brain = pe.Node(interface=fsl.MultiImageMaths(), name="T1wmulT2w_brain")
    ...
    meanbrainval = pe.Node(
        interface=fsl.ImageStats(), name="image_stats", iterfield=["in_file"]
    )
    ...
    T1wmulT2w_brain_norm = pe.Node(
        interface=fsl.ImageMaths(), name="T1wmulT2w_brain_norm"
    )
    ...
    preproc.connect(
        meanbrainval,
        ("out_stat", form_meanbrainval_string),
        T1wmulT2w_brain_norm,
        "op_string",
    )
    ...
    SmoothNorm = pe.Node(interface=fsl.ImageMaths(), name="SmoothNorm")
    SmoothNorm.inputs.op_string = (
        "-bin -s %f"
        % (
    ...
    T1wmulT2w_brain_norm_s_string.inputs.sigma = config.anatomical_preproc[
        "t1t2_bias_field_correction"
    ]["BiasFieldSmoothingSigma"]

    preproc.connect(SmoothNorm, "out_file", T1wmulT2w_brain_norm_s_string, "in_file")

    T1wmulT2w_brain_norm_s = pe.Node(
        interface=fsl.ImageMaths(), name="T1wmulT2w_brain_norm_s"
    )

    preproc.connect(T1wmulT2w_brain_norm, "out_file", T1wmulT2w_brain_norm_s, "in_file")
    preproc.connect(
        T1wmulT2w_brain_norm_s_string, "out_str", T1wmulT2w_brain_norm_s, "op_string"
    )
    ...
    T1wmulT2w_brain_norm_modulate = pe.Node(
        interface=fsl.MultiImageMaths(), name="T1wmulT2w_brain_norm_modulate"
    )
    T1wmulT2w_brain_norm_modulate.inputs.op_string = "-div %s"

    preproc.connect(
        T1wmulT2w_brain_norm, "out_file", T1wmulT2w_brain_norm_modulate, "in_file"
    )
    preproc.connect(
        T1wmulT2w_brain_norm_s,
        "out_file",
    ...
    STD = pe.Node(interface=fsl.ImageStats(), name="STD", iterfield=["in_file"])
    ...
    MEAN = pe.Node(interface=fsl.ImageStats(), name="MEAN", iterfield=["in_file"])
    ...
    T1wmulT2w_brain_norm_modulate_mask = pe.Node(
        interface=fsl.ImageMaths(), name="T1wmulT2w_brain_norm_modulate_mask"
    )
    ...
    preproc.connect(
        form_lower_string, "out_str", T1wmulT2w_brain_norm_modulate_mask, "op_string"
    )
    ...
    T1wmulT2w_brain_norm_modulate_mask_roi = pe.Node(
        interface=VolumeRemoveIslands(), name="remove_islands"
    )

    preproc.connect(
        T1wmulT2w_brain_norm_modulate_mask,
        "out_file",
        T1wmulT2w_brain_norm_modulate_mask_roi,
        "in_file",
    )
    ...
    bias_raw = pe.Node(interface=fsl.MultiImageMaths(), name="bias_raw")
    bias_raw.inputs.op_string = "-mas %s -dilall "

    preproc.connect(T1wmulT2w_brain_norm, "out_file", bias_raw, "in_file")
    preproc.connect(
        T1wmulT2w_brain_norm_modulate_mask_roi, "out_file", bias_raw, "operand_files"
    )

    # ${FSLDIR}/bin/fslmaths $WD/bias_raw.nii.gz -s $BiasFieldSmoothingSigma $OutputBiasField
    OutputBiasField = pe.Node(interface=fsl.ImageMaths(), name="OutputBiasField")
    OutputBiasField.inputs.op_string = (
        "-s %f "
        % (
            config.anatomical_preproc["t1t2_bias_field_correction"][
                "BiasFieldSmoothingSigma"
    ...
    OutputT1wRestoredBrainImage = pe.Node(
        interface=fsl.MultiImageMaths(), name="OutputT1wRestoredBrainImage"
    )
    ...
    preproc.connect(
        file_to_a_list, "out_list", OutputT1wRestoredBrainImage, "operand_files"
    )

    # ${FSLDIR}/bin/fslmaths $T1wImage -div $OutputBiasField $OutputT1wRestoredImage -odt float
    OutputT1wRestoredImage = pe.Node(
        interface=fsl.MultiImageMaths(), name="OutputT1wRestoredImage"
    )
    OutputT1wRestoredImage.inputs.op_string = "-div %s "

    preproc.connect(inputnode, "T1w", OutputT1wRestoredImage, "in_file")
    preproc.connect(
        OutputBiasField, "out_file", OutputT1wRestoredImage, "operand_files"
    )

    # ${FSLDIR}/bin/fslmaths $T2wImage -div $OutputBiasField -mas $T1wImageBrain $OutputT2wRestoredBrainImage -odt float
    OutputT2wRestoredBrainImage = pe.Node(
        interface=fsl.MultiImageMaths(), name="OutputT2wRestoredBrainImage"
    )
    ...
    preproc.connect(
        file_to_a_list, "out_list", OutputT2wRestoredBrainImage, "operand_files"
    )

    # ${FSLDIR}/bin/fslmaths $T2wImage -div $OutputBiasField $OutputT2wRestoredImage -odt float
    OutputT2wRestoredImage = pe.Node(
        interface=fsl.MultiImageMaths(), name="OutputT2wRestoredImage"
    )
    OutputT2wRestoredImage.inputs.op_string = "-div %s "

    preproc.connect(inputnode, "T2w", OutputT2wRestoredImage, "in_file")
    preproc.connect(
        OutputBiasField, "out_file", OutputT2wRestoredImage, "operand_files"
    )

    preproc.connect(OutputT1wRestoredImage, "out_file", outputnode, "T1w_biascorrected")
    preproc.connect(
        OutputT1wRestoredBrainImage, "out_file", outputnode, "T1w_brain_biascorrected"
    )
    preproc.connect(OutputT2wRestoredImage, "out_file", outputnode, "T2w_biascorrected")
    preproc.connect(
        OutputT2wRestoredBrainImage, "out_file", outputnode, "T2w_brain_biascorrected"
    )
...
def afni_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    inputnode_afni.inputs.set(
        mask_vol=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "mask_vol"
        ],
        shrink_factor=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "shrink_factor"
        ],
        var_shrink_fac=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "var_shrink_fac"
        ],
        shrink_fac_bot_lim=cfg.anatomical_preproc["brain_extraction"][
            "AFNI-3dSkullStrip"
        ]["shrink_factor_bot_lim"],
        avoid_vent=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "avoid_vent"
        ],
        ...
        fill_hole=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "fill_hole"
        ],
        NN_smooth=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "NN_smooth"
        ],
        smooth_final=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "smooth_final"
        ],
        avoid_eyes=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "avoid_eyes"
        ],
        use_edge=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "use_edge"
        ],
        exp_frac=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "exp_frac"
        ],
        push_to_edge=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "push_to_edge"
        ],
        use_skull=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "use_skull"
        ],
        perc_int=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "perc_int"
        ],
        max_inter_iter=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "max_inter_iter"
        ],
        fac=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"]["fac"],
        blur_fwhm=cfg.anatomical_preproc["brain_extraction"]["AFNI-3dSkullStrip"][
            "blur_fwhm"
        ],
    ...
    anat_skullstrip = pe.Node(
        interface=afni.SkullStrip(), name=f"anat_skullstrip_{pipe_num}"
    )
    ...
    anat_brain_mask = pe.Node(interface=afni.Calc(), name=f"anat_brain_mask_{pipe_num}")
...
def fsl_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    inputnode_bet.inputs.set(
        ...
        outline=cfg.anatomical_preproc["brain_extraction"]["FSL-BET"]["outline"],
        padding=cfg.anatomical_preproc["brain_extraction"]["FSL-BET"]["padding"],
        ...
        surfaces=cfg.anatomical_preproc["brain_extraction"]["FSL-BET"]["surfaces"],
        threshold=cfg.anatomical_preproc["brain_extraction"]["FSL-BET"]["threshold"],
    ...
    if strat_pool.check_rpool("desc-preproc_T1w"):
        ...
        outputs = {"space-T2w_desc-brain_mask": (anat_skullstrip, "mask_file")}
...
def niworkflows_ants_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    anat_skullstrip_ants = init_brain_extraction_wf(
        tpl_target_path=cfg.anatomical_preproc["brain_extraction"]["niworkflows-ants"][
            "template_path"
        ],
        tpl_mask_path=cfg.anatomical_preproc["brain_extraction"]["niworkflows-ants"][
            "mask_path"
        ],
        tpl_regmask_path=cfg.anatomical_preproc["brain_extraction"]["niworkflows-ants"][
            "regmask_path"
        ],
        name="anat_skullstrip_ants",
        atropos_use_random_seed=cfg.pipeline_setup["system_config"]["random_seed"]
        is None,
    )
    ...
    if strat_pool.check_rpool("desc-preproc_T1w"):
        outputs = {
            "space-T1w_desc-brain_mask": (
                ...
                "atropos_wf.copy_xform.out_mask",
            ),
...
def unet_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    unet_masked_brain = pe.Node(
        interface=fsl.MultiImageMaths(), name=f"unet_masked_brain_{pipe_num}"
    )
    ...
    wf.connect(unet_masked_brain, "out_file", native_brain_to_template_brain, "in_file")
    ...
    ants_template_head_to_template.inputs.convergence_threshold = [1.0e-8]
    wf.connect(
        native_head_to_template_head,
        "out_file",
    ...
    wf.connect(
        unet_masked_brain, "out_file", template_brain_to_native_brain, "reference"
    )
...
def freesurfer_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    fs_brain_mask_to_native = pe.Node(
        interface=freesurfer.ApplyVolTransform(), name="fs_brain_mask_to_native"
    )
    ...
    wf.connect(
        fs_brain_mask_to_native, "transformed_file", fs_brain_mask_to_nifti, "in_file"
    )

    # binarize the brain mask
    binarize_fs_brain_mask = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"binarize_fs_brainmask_{pipe_num}"
    )
    binarize_fs_brain_mask.inputs.args = "-bin"
    wf.connect(fs_brain_mask_to_nifti, "out_file", binarize_fs_brain_mask, "in_file")

    # fill holes
    fill_fs_brain_mask = pe.Node(
        interface=afni.MaskTool(), name=f"fill_fs_brainmask_{pipe_num}"
    )
    fill_fs_brain_mask.inputs.fill_holes = True
    fill_fs_brain_mask.inputs.outputtype = "NIFTI_GZ"
    wf.connect(binarize_fs_brain_mask, "out_file", fill_fs_brain_mask, "in_file")
...
def freesurfer_abcd_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    binary_mask = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"binarize_wmparc_{pipe_num}"
    )
    ...
    binary_filled_mask = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"binarize_filled_wmparc_{pipe_num}"
    )
    binary_filled_mask.inputs.args = "-bin"

    wf.connect(wb_command_fill_holes, "out_file", binary_filled_mask, "in_file")
...
def freesurfer_fsl_brain_connector(wf, cfg, strat_pool, pipe_num, opt):
    ...
    binarize_fs_brain = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"binarize_fs_brain_{node_id}"
    )
    ...
    wf.connect(reorient_fs_T1, "out_file", convert_head_to_template, "in_file")
    ...
    convert_xfm = pe.Node(
        interface=fsl_utils.ConvertXFM(), name=f"convert_xfm_{node_id}"
    )
    convert_xfm.inputs.invert_xfm = True

    wf.connect(convert_head_to_template, "out_matrix_file", convert_xfm, "in_file")

    # bet tmp_head_fs2standard.nii.gz tmp.nii.gz -f ${bet_thr_tight} -m
    skullstrip = pe.Node(interface=fsl.BET(), name=f"anat_BET_skullstrip_{node_id}")
    ...
    convert_template_mask_to_native = pe.Node(
        interface=fsl.FLIRT(), name=f"convert_template_mask_to_native_{node_id}"
    )
    ...
    wf.connect(
        convert_xfm, "out_file", convert_template_mask_to_native, "in_matrix_file"
    )

    wf.connect(reorient_fs_T1, "out_file", convert_template_mask_to_native, "reference")

    # fslmaths brain_fs_mask.nii.gz -add brain_fsl_mask_tight.nii.gz -bin brain_mask_tight.nii.gz
    # BinaryMaths doesn't use -bin!
    combine_mask = pe.Node(interface=fsl.BinaryMaths(), name=f"combine_mask_{node_id}")
    ...
    wf.connect(
        convert_template_mask_to_native, "out_file", combine_mask, "operand_file"
    )

    binarize_combined_mask = pe.Node(
        interface=fsl.maths.MathsCommand(), name=f"binarize_combined_mask_{node_id}"
    )
...
def mask_T2(wf_name="mask_T2"):
    ...
    inputnode = pe.Node(
        util.IdentityInterface(fields=["T1w", "T1w_mask", "T2w"]), name="inputspec"
    )

    outputnode = pe.Node(
        util.IdentityInterface(fields=["T1w_brain", "T2w_mask", "T2w_brain"]),
        name="outputspec",
    )

    # mask_t1w = 'fslmaths {t1w} -mas {t1w_mask_edit} {t1w_brain}'.format(**kwargs)
    mask_t1w = pe.Node(interface=fsl.MultiImageMaths(), name="mask_t1w")
    ...
    mask_t2w = pe.Node(interface=fsl.MultiImageMaths(), name="mask_t2w")
...
def acpc_align_head(wf, cfg, strat_pool, pipe_num, opt=None):
    acpc_align = acpc_alignment(
        config=cfg,
        acpc_target=cfg.anatomical_preproc["acpc_alignment"]["acpc_target"],
        mask=False,
...
def acpc_align_brain_with_mask(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {
        ...
        "space-T1w_desc-brain_mask": (acpc_align, "outputspec.acpc_brain_mask"),
        "space-T1w_desc-prebrain_mask": (
            strat_pool.get_data("space-T1_desc-brain_mask")
        ),
    }
...
def non_local_means(wf, cfg, strat_pool, pipe_num, opt=None):
    denoise = pe.Node(interface=ants.DenoiseImage(), name=f"anat_denoise_{pipe_num}")

    denoise.inputs.noise_model = cfg.anatomical_preproc["non_local_means_filtering"][
        "noise_model"
    ]
    ...
    outputs = {"desc-preproc_T1w": (denoise, "output_image")}
...
def n4_bias_correction(wf, cfg, strat_pool, pipe_num, opt=None):
    n4 = pe.Node(
        interface=ants.N4BiasFieldCorrection(dimension=3, copy_header=True),
        name=f"anat_n4_{pipe_num}",
    )
    n4.inputs.shrink_factor = cfg.anatomical_preproc["n4_bias_field_correction"][
        "shrink_factor"
    ]
...
def t1t2_bias_correction(wf, cfg, strat_pool, pipe_num, opt=None):
    t1t2_bias_correction = BiasFieldCorrection_sqrtT1wXT1w(
        config=cfg, wf_name=f"t1t2_bias_correction_{pipe_num}"
    )
...
def brain_mask_niworkflows_ants(wf, cfg, strat_pool, pipe_num, opt=None):
    wf, outputs = niworkflows_ants_brain_connector(wf, cfg, strat_pool, pipe_num, opt)
...
def brain_mask_acpc_niworkflows_ants(wf, cfg, strat_pool, pipe_num, opt=None):
    wf, wf_outputs = niworkflows_ants_brain_connector(
        wf, cfg, strat_pool, pipe_num, opt
    )

    outputs = {
        "space-T1w_desc-acpcbrain_mask": wf_outputs["space-T1w_desc-brain_mask"],
        "desc-preproc_T1w": wf_outputs["desc-preproc_T1w"],
    }
...
def brain_extraction(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    """
    anat_skullstrip_orig_vol = pe.Node(
        interface=afni.Calc(), name=f"brain_extraction_{pipe_num}"
    )
...
def brain_extraction_temp(wf, cfg, strat_pool, pipe_num, opt=None):
    anat_skullstrip_orig_vol = pe.Node(
        interface=afni.Calc(), name=f"brain_extraction_temp_{pipe_num}"
    )
...
def acpc_align_head_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {"desc-preproc_T2w": (acpc_align, "outputspec.acpc_aligned_head")}
...
def acpc_align_head_with_mask_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {
        ...
        "space-T2w_desc-brain_mask": (acpc_align, "outputspec.acpc_brain_mask"),
    }
...
def non_local_means_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    denoise = pe.Node(interface=ants.DenoiseImage(), name=f"anat_denoise_T2_{pipe_num}")

    node, out = strat_pool.get_data("desc-preproc_T2w")
    wf.connect(node, out, denoise, "input_image")

    outputs = {"desc-preproc_T2w": (denoise, "output_image")}
...
def n4_bias_correction_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {"desc-preproc_T2w": (n4, "output_image")}
...
def brain_mask_acpc_afni_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {"space-T2w_desc-acpcbrain_mask": wf_outputs["space-T2w_desc-brain_mask"]}
...
def brain_mask_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {"space-T2w_desc-brain_mask": (brain_mask_T2, "outputspec.T2w_mask")}
...
def brain_mask_acpc_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    outputs = {"space-T2w_desc-acpcbrain_mask": (brain_mask_T2, "outputspec.T2w_mask")}
...
def brain_extraction_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    if (
        cfg.anatomical_preproc["acpc_alignment"]["run"]
        and cfg.anatomical_preproc["acpc_alignment"]["acpc_target"] == "brain"
    ):
        outputs = {"desc-brain_T2w": (strat_pool.get_data(["desc-acpcbrain_T2w"]))}
    else:
        anat_skullstrip_orig_vol = pe.Node(
            interface=afni.Calc(), name=f"brain_extraction_T2_{pipe_num}"
        )
        ...
        outputs = {"desc-brain_T2w": (anat_skullstrip_orig_vol, "out_file")}
...
def brain_extraction_temp_T2(wf, cfg, strat_pool, pipe_num, opt=None):
    anat_skullstrip_orig_vol = pe.Node(
        interface=afni.Calc(), name=f"brain_extraction_temp_T2_{pipe_num}"
    )
    ...
    outputs = {"desc-tempbrain_T2w": (anat_skullstrip_orig_vol, "out_file")}
...
def freesurfer_abcd_preproc(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    brain_extraction = fnirt_based_brain_extraction(
    ...
    wf.connect(
        brain_extraction,
        "outputspec.anat_brain_mask",
        fast_correction,
        "inputspec.anat_brain_mask",
    )
    ...
    resample_head_1mm = pe.Node(
        interface=fsl.FLIRT(), name=f"resample_anat_head_1mm_{pipe_num}"
    )
    ...
    applywarp_head_to_head_1mm = pe.Node(
        interface=fsl.ApplyWarp(), name=f"applywarp_head_to_head_1mm_{pipe_num}"
    )
    ...
    wf.connect(resample_head_1mm, "out_file", applywarp_head_to_head_1mm, "ref_file")
    ...
    applywarp_brain_to_head_1mm = pe.Node(
        interface=fsl.ApplyWarp(), name=f"applywarp_brain_to_head_1mm_{pipe_num}"
    )
    ...
    wf.connect(
        ...
        applywarp_brain_to_head_1mm,
        "in_file",
    )

    wf.connect(resample_head_1mm, "out_file", applywarp_brain_to_head_1mm, "ref_file")

    # fslstats $T1wImageBrain -M
    average_brain = pe.Node(
        interface=fsl.ImageStats(), name=f"average_brain_{pipe_num}"
    )
    average_brain.inputs.op_string = "-M"
    average_brain.inputs.output_type = "NIFTI_GZ"

    wf.connect(
        fast_correction, "outputspec.anat_brain_restore", average_brain, "in_file"
    )
    ...
    wf.connect(applywarp_head_to_head_1mm, "out_file", normalize_head, "in_file")
...
def freesurfer_reconall(wf, cfg, strat_pool, pipe_num, opt=None):
    reconall = pe.Node(
        interface=freesurfer.ReconAll(), name=f"anat_freesurfer_{pipe_num}", mem_gb=2.7
    )
...
def fnirt_based_brain_extraction(config=None, wf_name="fnirt_based_brain_extraction"):
    ...
    outputnode = pe.Node(
        util.IdentityInterface(fields=["anat_brain", "anat_brain_mask"]),
        name="outputspec",
    )
    ...
    linear_reg = pe.Node(interface=fsl.FLIRT(), name="linear_reg")
    linear_reg.inputs.dof = 12
    linear_reg.inputs.interp = "spline"
    linear_reg.inputs.no_search = True

    preproc.connect(inputnode, "anat_data", linear_reg, "in_file")

    preproc.connect(inputnode, "template_skull_for_anat_2mm", linear_reg, "reference")
    ...
    preproc.connect(inputnode, "anat_data", non_linear_reg, "in_file")

    preproc.connect(
        inputnode, "template_skull_for_anat_2mm", non_linear_reg, "ref_file"
    )

    preproc.connect(linear_reg, "out_matrix_file", non_linear_reg, "affine_file")

    preproc.connect(
        inputnode, "template-ref-mask-res-2", non_linear_reg, "refmask_file"
    )
    ...
    preproc.connect(
        inputnode, "template_brain_mask_for_anat", apply_inv_warp, "in_file"
    )
    ...
    apply_mask = pe.Node(interface=fsl.MultiImageMaths(), name="apply_mask")
...
def fast_bias_field_correction(config=None, wf_name="fast_bias_field_correction"):
    ...
    inputnode = pe.Node(
        util.IdentityInterface(fields=["anat_data", "anat_brain", "anat_brain_mask"]),
        name="inputspec",
    )

    outputnode = pe.Node(
        util.IdentityInterface(
            fields=["anat_restore", "anat_brain_restore", "bias_field"]
        ),
        name="outputspec",
    )

    # fast -b -B -o ${T1wFolder}/T1w_fast -t 1 ${T1wFolder}/T1w_acpc_dc_brain.nii.gz
    fast_bias_field_correction = pe.Node(
        interface=fsl.FAST(), name="fast_bias_field_correction"
    )
    ...
    preproc.connect(inputnode, "anat_brain", fast_bias_field_correction, "in_files")

    preproc.connect(
        fast_bias_field_correction, "restored_image", outputnode, "anat_brain_restore"
    )

    preproc.connect(fast_bias_field_correction, "bias_field", outputnode, "bias_field")
    ...
    inverse_brain_mask = pe.Node(interface=fsl.ImageMaths(), name="inverse_brain_mask")
    inverse_brain_mask.inputs.op_string = "-mul -1 -add 1"

    preproc.connect(inputnode, "anat_brain_mask", inverse_brain_mask, "in_file")
    ...
    preproc.connect(inverse_brain_mask, "out_file", apply_mask, "operand_files")

    # fslmaths ${T1wFolder}/T1w_fast_restore.nii.gz -add ${T1wFolder}/T1w_acpc_dc_skull.nii.gz ${T1wFolder}/${T1wImage}_acpc_dc_restore
    anat_restore = pe.Node(interface=fsl.MultiImageMaths(), name="get_anat_restore")
    anat_restore.inputs.op_string = "-add %s"

    preproc.connect(
        fast_bias_field_correction, "restored_image", anat_restore, "in_file"
    )
...
def correct_restore_brain_intensity_abcd(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    merge_t1_acpc = pe.Node(interface=fslMerge(), name=f"merge_t1_acpc_{pipe_num}")
    ...
    multiply_t1_acpc_by_zero = pe.Node(
        interface=fsl.ImageMaths(), name=f"multiply_t1_acpc_by_zero_{pipe_num}"
    )
    ...
    convertwarp_orig_t1_to_t1 = pe.Node(
        interface=fsl.ConvertWarp(), name=f"convertwarp_orig_t1_to_t1_{pipe_num}"
    )
    ...
    threshold_biasfield = pe.Node(
        interface=fsl.ImageMaths(), name=f"threshold_biasfield_{pipe_num}"
    )
    ...
    wf.connect(convertwarp_orig_t1_to_t1, "out_file", applywarp_t1, "field_file")
    ...
    div_t1_by_biasfield = pe.Node(
        interface=fsl.ImageMaths(), name=f"div_t1_by_biasfield_{pipe_num}"
    )
    ...
    apply_mask = pe.Node(
        interface=fsl.maths.ApplyMask(), name=f"get_restored_corrected_brain_{pipe_num}"
    )
