...
def create_anat_preproc(use_AFNI, already_skullstripped=False):
    ...
    inputNode_AFNI = pe.Node(util.IdentityInterface(fields =['shrink_factor','var_shrink_fac','shrink_factor_bottom_lim','avoid_vent','niter','pushout','touchup','fill_hole','avoid_eyes','use_edges','exp_frac','smooth_final','push_to_edge','use_skull','perc_init','max_inter_init','blur_fwhm','fac']),name ='AFNI_options')
    ...
    if not already_skullstripped:
        if use_AFNI == True:
            ...
            anat_skullstrip.inputs.args = '-shrink_fac SF -var_shrink_fac -shrink_fac_bot_lim SFBL -avoid_vent -niter N_ITER -pushout -touchup -fill_hole R -NN_smooth NN_SM -avoid_eyes -use_edge -exp_frac -smooth_final SM -push_to_edge -use_skull -perc_init PERC_INIT -max_inter_iter -blur_fwhm FWHM -fac FAC'
        else:
    ...
    if not already_skullstripped:
        if use_AFNI == True:
            ...
            preproc.connect(inputNode_AFNI, ('shrink_factor','var_shrink_fac','shrink_factor_bottom_lim','avid_vent','niter','pushout','touchup','fill_hole','avoid_eyes','use_edges','exp_frac','smooth_final','push_to_edge','use_skull','perc_init','max_inter_init','blur_fwhm','fac') , anat_skullstrip,'args')
