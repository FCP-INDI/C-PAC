        ...
        if (self.listbox.GetChecked() or self.listbox.GetSelection()!= -1):
            ...
            for p in pipelines:
                ...
                if os.path.exists(pipeline):
                    ...
                    runGLA(pipeline, derv_path, p) #sublist, derv_path, p)
...
class runGLA(wx.Frame):
    ...
    def __init__(self, pipeline, path, name): #sublist, path, name):
        ...
        button2.Bind(wx.EVT_BUTTON, lambda event: \
                         self.onOK(event, pipeline) ) #sublist, pipeline) )
    ...
    def runAnalysis(self, pipeline, path): #sublist, path):
        try:
            import CPAC
            CPAC.pipeline.cpac_group_runner.run(pipeline, path)#sublist, path)
    ...
    def onOK(self, event, pipeline): #sublist, pipeline):
        ...
        if self.box1.GetValue():
            thread.start_new(self.runAnalysis, (pipeline, self.box1.GetValue())) #sublist, self.box1.GetValue()))
            self.Close()
        else:
