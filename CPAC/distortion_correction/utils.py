...
def phase_encode(
    ...
    if unwarp_dir in ["x", "x-", "-x", "i", "-i", "i-"]:
        if dwell_time_one and dwell_time_two:
            dim = nib.load(phase_one).shape[0]
            n_PE_steps = dim - 1
    ...
    dims = [
        int(
            subprocess.check_output(["fslval", phase_one, "dim4"]).decode(
                sys.stdout.encoding
            )
        ),
        int(
            subprocess.check_output(["fslval", phase_two, "dim4"]).decode(
                sys.stdout.encoding
            )
        ),
    ]
...
def z_pad(name="z_pad"):
    ...
    inputspec = pe.Node(
        util.IdentityInterface(fields=["input_image"]), name="inputspec"
    )
    ...
    fslroi = pe.Node(interface=fsl_utils.ExtractROI(), name=f"fslroi_{name}")
    fslroi.inputs.args = "0 -1 0 -1 0 1 0 -1"

    wf.connect(inputspec, "input_image", fslroi, "in_file")

    fslmaths = pe.Node(interface=fsl_maths.BinaryMaths(), name=f"fslmaths_{name}")
    ...
    files_list = pe.Node(interface=utility.Merge(2), name=f"create_list_{name}")

    wf.connect(inputspec, "input_image", files_list, "in1")
    wf.connect(fslmaths, "out_file", files_list, "in2")

    fslmerge = pe.Node(interface=fsl_utils.Merge(), name=f"fslmerge_{name}")
...
def run_fsl_topup(merged_file, acqparams):
    ...
    out_fieldcoef = os.path.join(os.getcwd(), f"{out_basename}_fieldcoef.nii.gz")

    out_movpar = os.path.join(os.getcwd(), f"{out_basename}_movpar.txt")

    corrected_outfile = os.path.join(os.getcwd(), f"{out_basename}_corrected.nii.gz")

    field_out = os.path.join(os.getcwd(), f"{out_basename}_field.nii.gz")

    out_jacs = [
        os.path.join(os.getcwd(), f"{jac_basename}_01.nii.gz"),
        os.path.join(os.getcwd(), f"{jac_basename}_02.nii.gz"),
    ]

    log_out = os.path.join(os.getcwd(), f"{out_basename}_log.log")

    out_xfms = [
        os.path.join(os.getcwd(), f"{xfm_basename}_01.mat"),
        os.path.join(os.getcwd(), f"{xfm_basename}_02.mat"),
    ]

    out_warps = [
        os.path.join(os.getcwd(), f"{warp_basename}_01.nii.gz"),
    ...
    cmd = [
        ...
        f"--imain={merged_file}",
        f"--out={out_basename}",
        f"--iout={corrected_outfile}",
        f"--fout={field_out}",
        f"--jacout={jac_basename}",
        f"--logout={log_out}",
        f"--rbmout={xfm_basename}",
