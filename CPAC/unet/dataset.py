...
class VolumeDataset(data.Dataset):
    def __init__(
        self, rimg_in=None, cimg_in=None, bmsk_in=None, transform=None, debug=True
    ):
...
class BlockDataset(data.Dataset):
    def __init__(self, rimg=None, bfld=None, bmsk=None, num_slice=3, rescale_dim=256):
        ...
        uns_rimg = nn.functional.interpolate(
            uns_rimg, scale_factor=rescale_factor, mode="trilinear", align_corners=False
        )
        rimg = torch.squeeze(uns_rimg, 0)

        if isinstance(bfld, torch.Tensor):
            uns_bfld = torch.unsqueeze(bfld, 0)
            uns_bfld = nn.functional.interpolate(
                uns_bfld,
        ...
        if isinstance(bmsk, torch.Tensor):
            ...
            uns_bmsk = nn.functional.interpolate(
                uns_bmsk, scale_factor=rescale_factor, mode="nearest"
            )
