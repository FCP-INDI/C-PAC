...
def Conv3dBlock(
    dim_in, dim_out, kernel_size=3, stride=1, padding=1, bias=True, use_bn=False
):
    if use_bn:
...
def UpConv3dBlock(dim_in, dim_out, kernel_size=4, stride=2, padding=1, bias=False):
    return nn.Sequential(
        nn.ConvTranspose3d(
            dim_in,
            dim_out,
...
def Conv2dBlock(
    dim_in, dim_out, kernel_size=3, stride=1, padding=1, bias=True, use_bn=True
):
    if use_bn:
        return nn.Sequential(
            nn.Conv2d(
...
def UpConv2dBlock(dim_in, dim_out, kernel_size=4, stride=2, padding=1, bias=True):
    return nn.Sequential(
        nn.ConvTranspose2d(
            dim_in,
            dim_out,
...
class UNet3d(nn.Module):
    def __init__(self, dim_in=1, num_conv_block=2, kernel_root=8, use_bn=False):
        ...
        for n in range(num_conv_block):
            if n == 0:
                setattr(
                    self,
                    "conv%d" % (n + 1),
                    Conv3dBlock(dim_in, kernel_root, use_bn=use_bn),
                )
            else:
                setattr(
                    self,
        ...
        for n in range(num_conv_block - 1):
            ...
            setattr(
                self,
                "upconv%dto%d" % (i + 1, i),
                UpConv3dBlock(kernel_root * (2**i), kernel_root * (2 ** (i - 1))),
            )
            setattr(
                self,
                "conv%dm" % (i),
                Conv3dBlock(kernel_root * (2**i), kernel_root * (2 ** (i - 1))),
            )
    ...
    def forward(self, x):
        ...
        for n in range(num_conv_block):
            if n == 0:
                ...
                conv_out["conv%d" % (n + 1)] = getattr(self, "conv%d" % (n + 1))(
                    self.max_pool(conv_out["conv%d" % n])
                )
        ...
        for n in range(num_conv_block - 1):
            ...
            tmp = torch.cat(
                (
                    getattr(self, "upconv%dto%d" % (i + 1, i))(
                        conv_out["conv%d" % (i + 1)]
                    ),
                    conv_out["conv%d" % (i)],
                ),
...
class UNet2d(nn.Module):
    def __init__(self, dim_in=6, num_conv_block=3, kernel_root=4, use_bn=True):
        ...
        for n in range(num_conv_block):
            if n == 0:
                setattr(
                    self,
                    "conv%d" % (n + 1),
                    Conv2dBlock(dim_in, kernel_root, use_bn=use_bn),
                )
            else:
                setattr(
                    self,
        ...
        for n in range(num_conv_block - 1):
            ...
            setattr(
                self,
                "upconv%dto%d" % (i + 1, i),
                UpConv2dBlock(kernel_root * (2**i), kernel_root * (2 ** (i - 1))),
            )
            setattr(
                self,
                "conv%dm" % (i),
                Conv2dBlock(kernel_root * (2**i), kernel_root * (2 ** (i - 1))),
            )
...
class MultiSliceBcUNet(nn.Module):
    def __init__(
        self, num_slice=6, in_shape=256, num_conv_block=4, kernel_root=16, use_bn=True
    ):
...
class MultiSliceSsUNet(nn.Module):
    def __init__(
        self, num_slice=6, in_shape=256, num_conv_block=5, kernel_root=16, use_bn=True
    ):
