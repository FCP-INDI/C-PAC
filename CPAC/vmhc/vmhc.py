...
def smooth_func_vmhc(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    smooth = spatial_smoothing(f"smooth_symmetric_{pipe_num}", fwhm, opt=opt)
...
def warp_timeseries_to_sym_template(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    apply_xfm = apply_transform(
        f"warp_ts_to_sym_template_{pipe_num}",
        reg_tool,
        time_series=True,
        num_cpus=num_cpus,
    ...
    outputs = {f"space-symtemplate_{resource}": (apply_xfm, "outputspec.output_image")}
...
def vmhc(wf, cfg, strat_pool, pipe_num, opt=None):
    ...
    copy_and_L_R_swap = pe.Node(
        interface=fsl.SwapDimensions(), name=f"copy_and_L_R_swap_{pipe_num}", mem_gb=3.0
    )
