...
def filter_subs(sub_paths, include_sites, include_subs, exclude_subs,
                site_idx, subj_idx):
    '''
    ...
    if include_sites is not None:
        ...
        for site in include_sites:
            keep_site_paths.extend([sp for sp in sub_paths if \
                                    sp.split('/')[site_idx] == site])
    else:
    ...
    # Include only
        ...
        if type(include_subs) is not list:
            ...
            for inc_sub in include_subs:
                keep_subj_paths.extend([kp for kp in keep_site_paths if \
                                        kp.split('/')[subj_idx] == inc_sub])
    # Or exclude only
    elif exclude_subs is not Nonde:
        keep_subj_paths = []
        if type(exclude_subs) is not list:
            exclude_subs = [exclude_subs]
            for exc_sub in exclude_subs:
                keep_subj_paths.extend([kp for kp in keep_site_paths if \
                                        kp.split('/')[subj_idx] != exc_sub])
    else:
...
def build_s3_sublist(data_confg_yml):
    '''
    ...
    creds_path, bucket_name, anat_fp, func_fp = return_s3_params(data_config_dict)
    ...
    sublist_out_yaml = data_config_dict['outputSubjectListLocation']

    # Filter out unwanted anat and func filepaths
    anat_s3_files = filter_sub_paths(anat_s3_files, include_sites,
                                     include_subs, exclude_subs,
                                     anat_site_idx, anat_subj_idx)

    func_s3_files = filter_sub_paths(func_s3_files, include_sites,
                                     include_subs, exclude_subs,
    ...
    with open(sublist_out_yaml, 'w') as out_sublist:
