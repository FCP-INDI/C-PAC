...
def load_data(subjects):
    ...
    subject_files = [subjects[i] for i in subject_ids]

    if subject_files[0].endswith(".csv"):
        data = np.array([np.genfromtxt(img).T for img in subject_files])
        voxel_masker = None

    else:
        images = [nib.load(img) for img in subject_files]
        voxel_masker = NiftiMasker()
        voxel_masker.fit(images)

        data = np.array([voxel_masker.transform(img).T for img in images])
...
def save_data_isc(subject_ids, ISC, p, out_dir, collapse_subj=True, voxel_masker=None):
    ...
    if voxel_masker:
        corr_file, p_file = (
            os.path.abspath("./correlations.nii.gz"),
            os.path.abspath("./significance.nii.gz"),
        )
        corr_out, p_out = (
            os.path.join(out_dir, "correlations.nii.gz"),
            os.path.join(out_dir, "significance.nii.gz"),
        )

        ISC_image = voxel_masker.inverse_transform(ISC.T)
        p_image = voxel_masker.inverse_transform(p.T)

        nib.save(ISC_image, corr_file)
        nib.save(p_image, p_file)

        os.makedirs(out_dir)
        nib.save(ISC_image, corr_out)
        nib.save(p_image, p_out)

    else:
        corr_file, p_file = (
            os.path.abspath("./correlations.csv"),
            os.path.abspath("./significance.csv"),
        )
        corr_out, p_out = (
            os.path.join(out_dir, "correlations.csv"),
            os.path.join(out_dir, "significance.csv"),
        )
...
def node_isc_permutation(permutation, D, masked, collapse_subj=True, random_state=0):
    ...
    permutation, min_null, max_null = isc_permutation(
...
def node_isfc_permutation(permutation, D, masked, collapse_subj=True, random_state=0):
    D = np.load(D)
    masked = np.load(masked)
    permutation, min_null, max_null = isfc_permutation(
        permutation, D, masked, collapse_subj, random_state
    )
    return permutation, min_null, max_null
...
def create_isc(name="isc", output_dir=None, working_dir=None, crash_dir=None):
    ...
    isc_node = pe.Node(
        Function(
            input_names=["D", "std", "collapse_subj"],
    ...
    permutations_node = pe.MapNode(
        Function(
            input_names=["permutation", "D", "masked", "collapse_subj", "random_state"],
            output_names=["permutation", "min_null", "max_null"],
            function=node_isc_permutation,
            as_module=True,
        ),
        name="ISC_permutation",
        iterfield="permutation",
    )

    significance_node = pe.Node(
        Function(
            input_names=["ISC", "min_null", "max_null", "two_sided"],
            output_names=["p"],
    ...
    wf.connect(
        [
            ...
            (
                inputspec,
                permutations_node,
                [(("permutations", _permutations), "permutation")],
            ),
...
def create_isfc(name="isfc", output_dir=None, working_dir=None, crash_dir=None):
    """
    ...
    save_node = pe.Node(
        Function(
            input_names=["subject_ids", "ISFC", "p", "out_dir", "collapse_subj"],
            output_names=["subject_ids", "correlations", "significance"],
