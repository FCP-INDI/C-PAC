...
def gather_nuisance(functional_file_path, selector, output_file_path, grey_matter_summary_file_path=None,
    ...
    for regressor_type in input_files_dict.keys():
    ...
    if regressor_type in selector and 'censor_method' in selector[regressor_type] and \
        ...
        try:
            ...
            raise ValueError("Could not read regressor {0} from {1}.".format(regressor_type, input_files_dict[regressor_type]))
        ...
        if len(censor_indices) > 0:
            ...
            # if number_of_previous_trs_to_remove and number_of_subsequent_trs_to_remove are not set,
            # assume they should be zero
    ...
    with open(output_file_path, "w") as ofd:
        ...
        for time_point in range(0, regressor_length):
            ...
            for regressor_index, regressor in enumerate(nuisance_regressors):
                row_values += ["{0:.8f}".format(regressor[time_point])]
...
def create_nuisance_workflow(pipeline_resource_pool,
    ...
    build_nuisance_regressors = pe.Node(Function(input_names=['functional_file_path',
                                                              'selector',
                                                              'output_file_path',
        ...
        if regressor_type in nuisance_configuration_selector and nuisance_configuration_selector[regressor_type]:
            ...
            if len(regressor_specification) != 2 or not regressor_specification[1]:
                ...
                if not (regressor_file_resource_key in pipeline_resource_pool and
                    ...
                    if "erosion" in regressor_descriptor and regressor_descriptor["erosion"] and \
                        ...
                        if not (mask_resample_file_resource_key in pipeline_resource_pool and
                            ...
                            if not (mask_file_resource_key in pipeline_resource_pool and
                                ...
                                if mask_file_resource_key.startswith('functional-variance'):

                                    create_variance_mask_node = pe.Node(
                                        Function(
                                            input_names=[
                                                'functional_data_file_path',
                                                'mask_file_path',
                                                'threshold',
                                                'output_file_name',
                                                'by_slice'
                                    ...
                                    create_variance_mask_node.inputs.output_file_name = 'variance_mask.nii.gz'
                                ...
                                anatomical_to_epi_transform_key = \
                                    "{0}_to_anat_linear_xform.mat".format(functional_specifier)
                                ...
                                mask_to_epi = pe.Node(interface=fsl.FLIRT(), name='mask_to_epi_flirt_applyxfm')
                                mask_to_epi.inputs.apply_isoxfm = regressor_descriptor["resolution"]
                                mask_to_epi.interface.estimated_memory_gb = 2.0

                                pipeline_resource_pool[mask_file_resource_key] = \
                                    (mask_to_epi, 'out_file')

                                nuisance.connect(*(pipeline_resource_pool[mask_file_resource_key] + (mask_to_epi, 'in_file')))
                                nuisance.connect(*(pipeline_resource_pool[functional_specifier] + (mask_to_epi, 'reference')))
                                nuisance.connect(*(pipeline_resource_pool[anatomical_to_epi_transform_key] + (mask_to_epi, 'in_matrix_file')))

                        if "erode_mask" in regressor_descriptor:

                            erode_mask_node = pe.Node(Function(input_names=['data'],
                                                               output_names=['eroded_data'],
                                                               function=erode_mask,
                                                               as_module=True),
                                                       name='erode_mask_node')

                            nuisance.connect(*(pipeline_resource_pool[mask_file_resource_key] + (erode_mask_node, 'data')))

                            pipeline_resource_pool[mask_resample_erode_file_resource_key] = \
                                (erode_mask_node, 'eroded_data')
                    ...
                    # add it to the specificaion
                    regressor_specification[1] = pipeline_resource_pool[regressor_file_resource_key]


    if 'Censor' in nuisance_configuration_selector and nuisance_configuration_selector['Censor']:
        regressor_map['Censor'] = ('censor_file_path', find_censors, 'out_file')

    # first check to see if we have any regressors, if so we need to combine them into a single file
    has_nuisance_regressors = any(
        nuisance_configuration_selector[1]
        for regressor_key in regressor_map
    )

    if has_nuisance_regressors:
        for regressor_key, regressor_val in regressor_map.items():
            if regressor_key in nuisance_configuration_selector and nuisance_configuration_selector[regressor_key]:
                
                if regressor_key is 'Censor' and nuisance_configuration_selector[regressor_key]['censor_method'] is not 'SpikeRegression':
                    continue

                nuisance.connect(regressor_val[1], regressor_val[2], build_nuisance_regressors, regressor_val[0])

    
    for regressor_type in ['WhiteMatter', 'GreyMatter', 'Ventricles']:
        if (regressor_type in nuisance_configuration_selector and nuisance_configuration_selector[regressor_type]):
            ...
            mask_csf_with_lat_ven = pe.Node(interface=afni.Calc(), name='mask_cfs_with_lat_ven')
            mask_csf_with_lat_ven.inputs.expr = 'step(a)*step(b)'
            mask_csf_with_lat_ven.inputs.outputtype = 'NIFTI_GZ'
            mask_csf_with_lat_ven.inputs.out_file = 'cfs_lat_ven_mask.nii.gz'

            nuisance.connect(inputspec, 'csf_mask_file_path', mask_csf_with_lat_ven, 'in_file_a')
            nuisance.connect(lat_ven_mni_to_anat, 'output_image', mask_csf_with_lat_ven, 'in_file_b')
            ...
            nuisance.connect(mask_csf_with_lat_ven, 'out_file', lat_ven_anat_to_2mm, 'in_file')
            nuisance.connect(mask_csf_with_lat_ven, 'out_file', lat_ven_anat_to_2mm, 'reference')
    ...
    if nuisance_configuration_selector.get('Censor'):
        if nuisance_configuration_selector['Censor']['censor_method'] in ['Kill', 'Zero', 'Interpolate']:
            if nuisance_configuration_selector['Censor']['censor_method'] is 'Interpolate':
                nuisance_regression.inputs.censor_mode = 'NTRP'
            else:
                nuisance_regression.inputs.censor_mode = nuisance_configuration_selector['Censor']['censor_method'].upper()
            nuisance.connect(find_censors, 'out_file', nuisance_regression, 'censor_file')
        else:
            raise ValueError('Censor method should be one of following: Kill, Zero, Interpolate.')


    if nuisance_configuration_selector.get('PolyOrt'):
        if not nuisance_configuration_selector['PolyOrt'].get('degree'):
            raise ValueError('Polynomial orthogonalization requested, but degree not provided.')

        nuisance_regression.inputs.orthogonalize_polynomial = nuisance_configuration_selector['PolyOrt']['degree']


    if nuisance_configuration_selector.get('Bandpass'):
        bottom_frequency = nuisance_configuration_selector['Bandpass'].get('bottom_frequency')
        if not nuisance_configuration_selector['Bandpass'].get('bottom_frequency'):
        ...
        if not nuisance_configuration_selector['Bandpass'].get('top_frequency'):
            raise ValueError('Bandpass filtering requested, but top_frequency not provided. Set to 9999 if you would '
                             'like a lowpass only filter')

        nuisance_regression.inputs.bandpass = [float(nuisance_configuration_selector['Bandpass']['bottom_frequency']),
                                               float(nuisance_configuration_selector['Bandpass']['top_frequency'])]
